...
def apply_reviews(review_request, reviews):
    ...
    for review in review_request["depends_on"]:
        ...
        apply_reviews(ReviewBoardHandler.api(review_url)["review_request"], reviews)
...
def verification_needed_write(review_ids, parameters):
    ...
    print("There are %s review requests that need verification" % num_reviews)
...
def main():
    ...
    query_string = urllib.parse.urlencode(json.loads(parameters.query)).replace("%3A", ":")
